...
then 
  ...
  do 
    client_port=$(( 31820+${i} ))
    server_port=$(( 31860+${i} ))
    echo "sudo ./build/client -data_len=${data_len} -total_request=${request} -port=${server_port} -log_file=record${i}.log"
    sudo ./build/client -data_len=${data_len} -total_request=${request} -port=${server_port} -log_file=record${i}.log &
  done
fi
