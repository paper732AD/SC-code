...
#include "huibuffer/huibuffer.h"
#include "huibuffer/ctrl_buffer.h"
...
class Rpc {
 ...
 private:
  ...
  static constexpr size_t kInitialHugeAllocSize = MB(200);
 ...
 public:
  ...
  inline Buffer alloc_buffer (size_t max_data_size){
    assert(max_data_size > 0); 
    lock_cond(&huge_alloc_lock_);
    // * 分配空间,数据大小，加上包头的大小
    Buffer buffer =
        huge_alloc_->alloc(max_data_size);// + sizeof(pkthdr_t));
    unlock_cond(&huge_alloc_lock_);
    if (unlikely(buffer.buf_ == nullptr)) {
      ERPC_ERROR("huge_alloc_->alloc failed!\n");
    }
    return buffer;
  }
  ...
  inline void free_msg_buffer(Buffer buffer) {
    lock_cond(&huge_alloc_lock_);
    huge_alloc_->free_buf(buffer);
    unlock_cond(&huge_alloc_lock_);
  }
  ...
  void enqueue_request(int session_num, uint8_t req_type, Serializer *req_msgbuf,
                       ...
                       void *tag, uint32_t _lkey=1, pkthdr_t *pkthdr = nullptr, void* context=nullptr, size_t cont_etid = kInvalidBgETid);


  void enqueue_response(ReqHandle *req_handle, Serializer *resp_msgbuf, uint32_t _lkey=-1, pkthdr_t * pkthdr=nullptr, void* context=nullptr);
  ...
  // TODO 这个行数要进行修改，改成模板，根据请求数量来回复 protocol.h 的结构体
 ...
 private:
  ...
  inline void bury_req_msgbuf_server_st(SSlot *sslot) {
    // delete sslot->server_info_.req_msgbuf_;
...
private:
  ...
  static inline size_t wire_pkts(int req_pkg_num,
                                 int resp_pkg_num) {
    return req_pkg_num + resp_pkg_num - 1;
  }

  /// Return true iff this sslot needs to send more request packets
  static inline bool req_pkts_pending(SSlot *sslot) {
    return sslot->client_info_.num_tx_ < sslot->tx_msgbuf_->total_pkg;
  }
  ...
  void drain_tx_batch_and_dma_queue() {
    if (tx_batch_i_ > 0) {
      ERPC_INFO("void drain_tx_batch_and_dma_queue()\n");
      do_tx_burst_st();
    }
    transport_->tx_flush();
  }
  ...
  /// TX burst; credits are used in both cases.
  void kick_rfr_st(SSlot *,const pkthdr_t* pkthdr);
  ...
  void process_small_req_st(SSlot *, pkthdr_t *, uint8_t *);

  /// Process a packet for a multi-packet request
  void process_large_req_one_st(SSlot *, pkthdr_t *, uint8_t *);

  /**
   * @brief Process a single-packet response
   * @param rx_tsc The timestamp at which this packet was received
   */
  void process_resp_one_st(SSlot *, const pkthdr_t *, size_t rx_tsc, uint8_t *body);
  ...
  inline void enqueue_pkt_tx_burst_st(SSlot *sslot, size_t pkt_idx,
                                      size_t *tx_ts) {
    assert(in_dispatch());
    const Serializer *tx_msgbuf = sslot->tx_msgbuf_;
    ERPC_INFO("it is %d iovec in tx_msgbuf\n",tx_msgbuf->iov.iovcnt());

    // * 注意这里是引用咯
    // ? 这里 tx_batch_i_ 不需要用锁吗？
    Transport::tx_burst_item_t &item = tx_burst_arr_[tx_batch_i_];

    item.routing_info_ = sslot->session_->remote_routing_info_;
    // * 指针指向
    item.msg_buffer_ = const_cast<Serializer *>(tx_msgbuf);
    ERPC_INFO("it is %d iovec in item.msg_buffer_\n",item.msg_buffer_->iov.iovcnt());
    item.pkt_idx_ = pkt_idx; // 不是包的数量，而是现在包的索引
    ERPC_INFO("index is %d\n",pkt_idx);
    ...
    ERPC_INFO("Rpc %u, lsn %u (%s): TX !!!. Slot %s.%s\n", rpc_id_,
    ...
    if (tx_batch_i_ == TTr::kPostlist) {
      ERPC_INFO("tx_batch_i_ = %d, do_tx_burst_st();\n", tx_batch_i_);
      do_tx_burst_st();
    }
  }
  ...
  inline void enqueue_hdr_tx_burst_st(SSlot *sslot, Serializer *ctrl_msgbuf,
                                      size_t *tx_ts) {
    assert(in_dispatch());
    ...
    //            sslot->progress_str().c_str(), item.drop_ ? " Drop." : "");
    tx_batch_i_++;
    ERPC_INFO("enqueue_hdr_tx_burst_st tx_batch_i_++ is %u\n",tx_batch_i_);

    // 因为这里没发送，返回了，导致之前的指针也因为函数结束而被释放，最终导致指针指向了被释放的内存空间。
    if (tx_batch_i_ == TTr::kPostlist) {
      ERPC_INFO("tx_batch_i_ == TTr::kPostlist\n");
      do_tx_burst_st();
    }
  }
  ...
  inline void enqueue_wheel_req_st(SSlot *sslot, size_t pkt_num) {
    ...
    size_t pktsz = sizeof(pkthdr_t) + sslot->tx_msgbuf_->iov.sum();
  ...
  inline void enqueue_wheel_rfr_st(SSlot *sslot, size_t pkt_num) {
    ...
    const size_t pkt_idx = resp_ntoi(pkt_num, 1);
    const void *resp_msgbuf = sslot->client_info_.resp_msgbuf_;
    size_t pktsz = sizeof(pkthdr_t) + 20;
    size_t ref_tsc = dpath_rdtsc();
    size_t desired_tx_tsc =
        sslot->session_->cc_getupdate_tx_tsc(ref_tsc, pktsz);

    ERPC_CC("inline void enqueue_wheel_rfr_st(SSlot *sslot, size_t pkt_num) failed revoke");
  ...
  inline void do_tx_burst_st() {
    ...
    // * 应该是这里发出去。
    ERPC_INFO("transport_->tx_burst(arr, %d)\n", tx_batch_i_);
  ...
  static inline void bump_credits(Session *session) {
    ERPC_INFO("in void bump_credits(Session *session)\n");
    assert(session->is_client());
    assert(session->client_info_.credits_ < kSessionCredits);
    // ERPC_WARN("session->client_info_.credits_++ = %d\n",session->client_info_.credits_+1);
...
private:
  ...
  uint8_t *rx_ring_[TTr::kNumRxRingEntries][2];//[0] is header [1] is data_body
  ...
  pkthdr_t* ctrl_msgbufs_hdr;///< Buffers for RFR/CR
  Serializer ctrl_serialization[2 * TTr::kUnsigBatch];
