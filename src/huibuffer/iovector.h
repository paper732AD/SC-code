...
//    size_t size() const;
    ...
    {
        auto& f = front();
        // std::cout << "f.iov_len = "  << f.iov_len  << std::endl;
...
//    }
    ...
    uint16_t back_free_iovcnt()
    {
        do_assert();
        if(capacity == 0)
            capacity = 2300;
        ////ERPC_INFO("back_free_iovcnt, capacity=%d, iov_end=%d, return %d\n",capacity - iov_end);
    ...
    void* extract_front_continuous(size_t bytes)
    {
        ...
        if (ptr){
            if(front().iov_len == 0){
                // ERPC_INFO("front().iov_len == 0\n");
                iov_begin+=1;
            }
            // ERPC_INFO("return ptr = 0x%X\n", uint64_t(ptr));
            return ptr;
        }
    ...
    void* extract_back_continuous(size_t bytes)
    {
        ...
        auto buf = do_malloc(bytes);
        // //ERPC_INFO("iovector: buf = do_malloc(bytes); buf = 0x%X\n", buf);
        auto ret = extract_back(bytes, buf);
        // //ERPC_INFO("iovector: auto ret = extract_back(bytes, buf); bytes= %d, ret = %d\n",bytes, ret);
        return ret == bytes ?
            buf :
            nullptr;
    }
...
typedef IOVectorEntity<2300, 8> IOVector;

inline iovector* new_iovector(uint16_t capacity, uint16_t preserve)
{
    auto size = sizeof(iovector) + sizeof(iovec) * capacity + sizeof(void*) * capacity ;
...
// clone iovecs only when CAPACITY is not enough
