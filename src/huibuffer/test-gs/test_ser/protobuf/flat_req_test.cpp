...
void ser_func (FlatLogFileFlushChunkRequestPB & temp){
    // temp.SerializeToString(&str);
    temp.SerializePartialToArray(_str,temp.ByteSize());
}

void  dser_func (FlatLogFileFlushChunkRequestPB &resp){
...
int main(int argc, char**argv) {
    ...
    FlatLogFileFlushChunkRequestPB temp,resp;
    temp = get_flat_req(FLAGS_total_len);
...
FlatLogFileFlushChunkRequestPB get_flat_req(int FLAGS_data_len){
    str = "";
    for(int i=0;i<FLAGS_data_len/2/3;i++)
        str+="as";
