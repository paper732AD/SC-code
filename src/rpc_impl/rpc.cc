...
Rpc<TTr>::Rpc(Nexus *nexus, void *context, uint8_t rpc_id,
  ...
  Buffer _vctrl_msgbufs_hdr = this->alloc_buffer(sizeof(pkthdr_t)*2*TTr::kUnsigBatch);
  ctrl_msgbufs_hdr =  new(_vctrl_msgbufs_hdr.buf_) pkthdr_t[2*TTr::kUnsigBatch];
  for(int i=0;i<2*TTr::kUnsigBatch;++i){
    ctrl_serialization[i].iov.iov_begin = ctrl_serialization[i].iov.iov_end = 1;
    (ctrl_msgbufs_hdr+i)->lkey = _vctrl_msgbufs_hdr.lkey_;
    ctrl_serialization[i].iov.push_front(ctrl_msgbufs_hdr+i, sizeof(pkthdr_t));
  }
