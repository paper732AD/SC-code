...
void Rpc<TTr>::pkt_loss_scan_st() {
  ...
  while (cur != &active_rpcs_tail_sentinel_) {
    ...
    if (ev_loop_tsc_ - cur->client_info_.progress_tsc_ > rpc_rto_cycles_) {
      ERPC_INFO("pkt_loss_retransmit_st(); in Package_loss_scan()\n");
      pkt_loss_retransmit_st(cur);
      ERPC_INFO("drain_tx_batch_and_dma_queue(); in Package_loss_scan()\n");
      drain_tx_batch_and_dma_queue();
    }
...
void Rpc<TTr>::pkt_loss_retransmit_st(SSlot *sslot) {
  ...
  ERPC_REORDER("%s: Retransmitting %s.\n", "issue_msg",
               ci.num_rx_ < 1 ? "requests" : "RFRs");
  credits += delta;
  ci.num_tx_ = ci.num_rx_;
  ci.progress_tsc_ = ev_loop_tsc_;

  if(req_pkts_pending(sslot)) {
    ERPC_INFO("req_pkts_pending(sslot) is true, kick_req_st() to send request\n");
    kick_req_st(sslot);
   }
   else {
    ERPC_INFO("req_pkts_pending(sslot) is false, kick_rfr_st() to send a rfr\n");
    // kick_rfr_st(sslot);
   }
}
