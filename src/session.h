...
// TODO 只能把这里改成模板了
// TODO 用两个模板变量替换掉MsgBuffer的类型
struct enq_req_args_t {
  int session_num_;
  uint8_t req_type_;
  Serializer *req_msgbuf_;
  void* resp_msgbuf_;
  ...
  enq_req_args_t() {}
  enq_req_args_t(int session_num, uint8_t req_type, Serializer *req_msgbuf,
                 void *resp_msgbuf, erpc_cont_func_t cont_func, void *tag,
                 size_t cont_etid)
...
struct enq_resp_args_t {
  ...
  Serializer *resp_msgbuf_;

  enq_resp_args_t() {}
  enq_resp_args_t(ReqHandle *req_handle, Serializer *resp_msgbuf)
...
class Session {
 ...
 private:
  template<class T>
  Session(T *rpc, Role role, conn_req_uniq_token_t uniq_token, double freq_ghz,
