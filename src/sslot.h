...
class SSlot {
 ...
 public:
  ...
  Serializer pre_resp_msgbuf_;
  uint8_t* pre_req;// gs define this for smr
  /// A non-preallocated msgbuf for possibly multi-packet responses
  Serializer dyn_resp_msgbuf_;
...
public:
  union {
    struct {
      void **resp_msgbuf_;      ///< User-supplied response buffer
