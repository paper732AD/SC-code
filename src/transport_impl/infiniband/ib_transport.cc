...
void IBTransport::init_hugepage_structures(HugeAlloc *huge_alloc,
                                           uint8_t *rx_ring[kNumRxRingEntries][2]) {
...
void IBTransport::init_verbs_structs() {
  ...
  create_attr.cap.max_send_sge = 60;
  create_attr.cap.max_recv_sge = 2;
  ...
  if (probe_ret != kModdedProbeRet) {
    ERPC_WARN("Modded driver unavailable. Fast recv is unavailable. Performance will be low.\n");
    use_fast_recv = false;
  } else {
    ERPC_WARN("Modded driver available. Fast recv is available.\n");
    use_fast_recv = true;
  }
}
...
void IBTransport::init_recvs(uint8_t *rx_ring[kNumRxRingEntries][2]) {
  std::ostringstream xmsg;  // The exception message
  ...
  constexpr size_t offset_hdr = 0;
  constexpr size_t GRH_HDR_SIZE = sizeof(pkthdr_t)+64/*GHR*/;
  constexpr size_t offset_body = (sizeof(pkthdr_t)+64/*GHR*/)*kRQDepth;
  constexpr size_t body_size = kMaxDataPerPkt; 
  ...
  for (size_t i = 0; i < kRQDepth; i++) {
    ...
    const size_t _offset_hdr = offset_hdr + (i * GRH_HDR_SIZE) ;
    // const size_t _offset_hdr = offset_hdr + (i * sizeof(pkthdr_t));
    const size_t _offset_body = offset_body + (i * body_size);

    recv_sgl[i][0].length = GRH_HDR_SIZE-24;
    // recv_sgl[i][0].length = sizeof(pkthdr_t);
    recv_sgl[i][0].lkey = ring_extent.lkey_;
    recv_sgl[i][0].addr = reinterpret_cast<uint64_t>(&buf[_offset_hdr]);
    recv_sgl[i][1].length = body_size;
    recv_sgl[i][1].lkey = ring_extent.lkey_;
    recv_sgl[i][1].addr = reinterpret_cast<uint64_t>(&buf[_offset_body]);
    

    recv_wr[i].wr_id = recv_sgl[i][0].addr + kGRHBytes;  // For quick prefetch
    recv_wr[i].sg_list = &recv_sgl[i][0];
    recv_wr[i].num_sge = 2;

    // Circular link
    recv_wr[i].next = (i < kRQDepth - 1) ? &recv_wr[i + 1] : &recv_wr[0];
    rx_ring[i][0] = &buf[_offset_hdr + kGRHBytes];  // RX ring entry
    // rx_ring[i][0] = &buf[_offset_hdr];  // RX ring entry
    rx_ring[i][1] = &buf[_offset_body];  // RX ring entry
  }
  ...
  rt_assert(ret == 0, "Failed to fill RECV queue. while init_recvs");
