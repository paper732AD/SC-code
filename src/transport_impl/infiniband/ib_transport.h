...
class IBTransport : public Transport {
 public:
  ...
  // ever beneficial?
  static constexpr size_t kMTU = kIsRoCE ? 4096 : 3840;
  ...
  void init_hugepage_structures(HugeAlloc *huge_alloc, uint8_t *rx_ring[kNumRxRingEntries][2]);
 ...
 private:
  ...
  void init_recvs(uint8_t *rx_ring[kNumRxRingEntries][2]);
  ...
  struct ibv_sge send_sgl[kPostlist][16];  ///< SGEs for eRPC header & payload
  ...
  struct ibv_sge recv_sgl[kRQDepth][2];
