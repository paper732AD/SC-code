...
void send_client_response(AppContext *c, erpc::ReqHandle *req_handle,
  ...
  erpc::Buffer _vresp_msgbuf = c->rpc->alloc_buffer(sizeof(client_resp_t));
  client_resp_t *_client_resp = new(_vresp_msgbuf.buf_) client_resp_t();
  *_client_resp = client_resp;
  erpc::Serializer &resp_msgbuf = req_handle->pre_resp_msgbuf_;
  resp_msgbuf.clear();
  resp_msgbuf.iov.push_back(_client_resp, sizeof(client_resp_t));
  int total_pkt = resp_msgbuf.init_data_iov(erpc::IBTransport::kMaxDataPerPkt);
  erpc::Buffer _vhdr = c->rpc->alloc_buffer(sizeof(erpc::pkthdr_t)*total_pkt);
  erpc::pkthdr_t* hdr = new(_vhdr.buf_) erpc::pkthdr_t[total_pkt];
  hdr->total_pkt = total_pkt;
  hdr->lkey = _vhdr.lkey_;

  // important
  c->rpc->enqueue_response(req_handle, &resp_msgbuf, _vhdr.lkey_, hdr, c);
}

void client_req_handler(erpc::ReqHandle *req_handle, void *_context) {
  ERPC_INFO("In client_req_handler\n");
