...
void send_req_one(AppContext *c) {
  ...
  erpc::Buffer _req = c->rpc->alloc_buffer(sizeof(client_req_t)); 
  client_req_t *req = new(_req.buf_) client_req_t();
  erpc::Serializer& reqser = c->client.req_msgbuf;
  reqser.clear();
  ...
  reqser.iov.push_back(req, sizeof(*req));
  int total_pkg = reqser.init_data_iov(erpc::IBTransport::kMaxDataPerPkt);
  erpc::Buffer _hdr = c->rpc->alloc_buffer(sizeof(erpc::pkthdr_t)*total_pkg);
  erpc::pkthdr_t *hdr = new(_hdr.buf_) erpc::pkthdr_t[total_pkg];
  hdr->total_pkt = total_pkg;
...
// important ~
  c->rpc->enqueue_request(
      ...
      uint32_t(_req.lkey_), hdr, c);
}

void client_cont(void *_sslot, void *) {
  // ERPC_WARN("In client_cont\n");
  erpc::SSlot *s = static_cast<erpc::SSlot *>(_sslot);
  AppContext* c = static_cast<AppContext *>(s->c);
  ...
  if (c->client.num_resps_this_measurement == 1000) {
  ...
  erpc::Serializer& resp = s->dyn_resp_msgbuf_;
  if (likely(resp.iov.sum() > 0)) {
    // The RPC was successful
    auto *client_resp =
        reinterpret_cast<client_resp_t *>(resp.iov[0].iov_base);
...
void client_func(erpc::Nexus *nexus, AppContext *c) {
  ...
  // erpc::Buffer _req = c->rpc->alloc_buffer(sizeof(client_req_t));
  // client_req_t* req = new(_req.buf_) client_req_t();
  // erpc::Serializer req_ser;
  // req_ser.clear();
  // req_ser.iov.push_back(req, sizeof(*req));
  // int total_pkg = req_ser.init_data_iov(erpc::IBTransport:kMaxDataPerPkt);
  // erpc::Buffer _hdr = c->rpc->alloc_buffer(sizeof(erpc::pkthdr_t)*total_pkg);
  // erpc::pkthdr_t hdr = new(_hdr.buf_) erpc::pkthdr_t[total_pkg];

  // Pre-allocate MsgBuffers
  // c->client.req_msgbuf = c->rpc->alloc_msg_buffer_or_die(sizeof(client_req_t));
  // c->client.resp_msgbuf = c->rpc->alloc_msg_buffer_or_die(sizeof(client_resp_t));
